...
export function HardwareSkinTemperatureChart({
  ...
  const { chartSource, chartConfig, configCount } =
    transformHardwareStatusSourceToSkinTemperatureChart(hardwareStatusSource);
...
/**
 * ハードウェアステータスソースを表面温度のチャートソースに変換
 * @param hardwareStatusSource ハードウェアステータスソース
 * @returns チャートのデータと設定
 */
const transformHardwareStatusSourceToSkinTemperatureChart = (
  hardwareStatusSource: HardwareStatusSourceType
) => {
  const configKey = "表面";
  let configCount = 0;
